...
// Get flight data grouped by week
// 🔢 Pull weekly trend data from wp_flight_logs
$trendStmt = $conn->prepare("
  SELECT 
    DATE_FORMAT(start_time, '%Y-%u') as flight_week,
    appointment_type as flight_type,
    COUNT(*) as count
  FROM wp_flight_logs
  WHERE status = 'Completed'
    AND start_time >= CURRENT_DATE() - INTERVAL 90 DAY
  GROUP BY flight_week, appointment_type
");
$trendStmt->execute();
$trendData = $trendStmt->fetchAll(PDO::FETCH_ASSOC);


// 1. Create last 9 week labels
$labels = [];
$weekKeys = [];
$now = new DateTime();
for ($i = 8; $i >= 0; $i--) {
  $week = (clone $now)->modify("-$i week")->format("Y-W");
  $labels[] = "Week $week";
  $weekKeys[] = $week;
}

// 2. Define types from your dropdown
$types = [
  'Dual Training', 'Dual Cross Country', 'Solo Training Flight',
  'Solo Cross Country', 'Rental Solo - Local', 'Rental Cross Country',
  'Discovery Flight', 'Checkride', 'Ground Lesson'
];
$series = [];
foreach ($types as $type) {
  $series[$type] = array_fill(0, count($weekKeys), 0);
}

// 3. Fill data into buckets
foreach ($trendData as $row) {
  $week = str_replace('-', '-', $row['flight_week']);
  $type = $row['flight_type'];
  $count = (int)$row['count'];
  $index = array_search($week, $weekKeys);
  if ($index !== false && isset($series[$type])) {
    $series[$type][$index] = $count;
  }
}
...
<body class="bg-light">
  ...
  <div class="container mt-4">
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="list-group mb-4 shadow-sm">
          <a href="admin-panel.php" class="list-group-item list-group-item-action active">🏠 Dashboard</a>
          <a href="company-settings.php" class="list-group-item list-group-item-action">🛠 Company Settings</a>
          <a href="admin-student-management.php" class="list-group-item list-group-item-action">🎓 Students</a>
          <a href="manage-cfis.php" class="list-group-item list-group-item-action">🧑‍🏫 CFIs</a>
          <a href="manage-wings.php" class="list-group-item list-group-item-action">🪶 Manage Wings</a>
          <a href="../calendar.v2/manage-aircraft.php" class="list-group-item list-group-item-action">✈️ Aircraft</a>
          <a href="payroll-center.php" class="list-group-item list-group-item-action">💰 CFI Payroll Console</a>
          <a href="flight-schedule.php" class="list-group-item list-group-item-action">🗓️ Schedule</a>
          <a href="billing.php" class="list-group-item list-group-item-action">💳 Billing</a>
          <a href="admin-audit-logs.php" class="list-group-item list-group-item-action">📋 Logs</a>
          <a href="email-log.php" class="list-group-item list-group-item-action">📬 Email Log</a>
    ...
    <!-- 📊 Flights Per Day Trend Chart -->
<div class="card shadow-sm mt-4">
  <div class="card-body">
    <h5 class="card-title">📈 Flights Per Day (Last 60 Days)</h5>
    <canvas id="flightsTrendChart" height="120"></canvas>
  </div>
</div>
...
const datasets = [
  <?php foreach ($series as $type => $data): ?>
    {
      label: "<?php echo $type; ?>",
      data: <?php echo json_encode($data); ?>,
      borderColor: "<?php echo '#' . substr(md5($type), 0, 6); ?>",
      backgroundColor: "<?php echo '#' . substr(md5($type . 'bg'), 0, 6); ?>",
      borderWidth: 2,
      fill: false,
      tension: 0.3
    },
...
new Chart(document.getElementById('flightsTrendChart'), {
  type: 'line',
  data: {
    labels: chartLabels,
    datasets: datasets
