...
try {
  ...
  $tail_number = strtoupper(trim($input['tail_number'] ?? ''));
  ...
  // ✅ Pull flight_time_factor from wp_aircraft
  $stmt = $db->prepare("SELECT flight_time_factor FROM wp_aircraft WHERE tail_number = ?");
  $stmt->execute([$tail_number]);
  $aircraft = $stmt->fetch();
  $plane_factor = $aircraft ? floatval($aircraft['flight_time_factor']) : 1.00;

  // ✅ Pull student balances
  $prev_aircraft_hours = floatval($student['aircraft_hours_remaining']);
  $prev_instructor_hours = floatval($student['instructor_hours_remaining']);

  // ✅ Insert into wp_flight_logs with new balance fields
  $stmt = $db->prepare("INSERT INTO wp_flight_logs (
    ...
    status,
    checkout_time,
    previous_aircraft_hours_remaining,
    previous_instructor_hours_remaining
