...
function getNextRecommendedFlights($studentId, $tailNumber, $cfiId, $db, $isSolo = false) {
  ...
  foreach ($topPatterns as $pattern) {
    ...
    for ($i = 1; $i <= 10; $i++) {
      ...
      if (date('N', strtotime($date)) != $dayOfWeek) continue;
      ...
      if (!$isSolo) $params[':cfiId'] = $cfiId;

      $conflictCheck->execute($params);
      if ($conflictCheck->fetchColumn() > 0) continue;
      ...
      if (!$isSolo && $cfiId) {
        ...
        if ($cfiRow) $cfiName = $cfiRow['first_name'] . ' ' . $cfiRow['last_name'];
      }
...
try {
  ...
  $row = $stmt->fetch();
  if (!$row) throw new Exception("Flight log not found.");
  ...
  $totalFlightTime = max(0, $endHobbs - $startHobbs);
...
$totalToday += $totalFlightTime; // Include this flight, which is not marked 'Completed' yet
  ...
  if ($studentId) {
    ...
    if (in_array($normalizedType, ['dual training', 'dual cross country', 'checkride'])) {
      ...
      ");
      $deductCFI->execute([
        ':instructor' => $totalFlightTime,
        ':sid' => $studentId
      ]);
    }

    // ✅ Fetch updated student hours for logging
    $getStudent = $db->prepare("SELECT aircraft_hours_remaining, instructor_hours_remaining FROM wp_students WHERE student_id = ?");
