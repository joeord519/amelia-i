...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($type === 'email') {
    ...
    if ($program && $subject && $html) {
      ...
      $feedback = "<div class='alert alert-success'>✅ Sent email to {$count} lead(s) in '{$program}'</div>";
    } else {
      $feedback = "<div class='alert alert-danger'>❌ Please complete all email fields.</div>";
    }
  }
  ...
  if ($type === 'sms') {
    ...
    if ($program && $message) {
      ...
      foreach ($leads as $lead) {
        ...
        if ($sent) {
          logCommHistory($lead['id'], 'sms', 'outbound', $cleaned);
          $count++;
        }
      }
      $feedback = "<div class='alert alert-success'>✅ Sent SMS to {$count} lead(s) in '{$program}'</div>";
    } else {
      $feedback = "<div class='alert alert-danger'>❌ Please complete all SMS fields.</div>";
    }
  }
}
    ...
    <div class="tab-pane fade show active" id="emailTab">
      <form method="POST">
        ...
        <div class="mb-3">
          ...
          <select name="program" class="form-select" required>
            ...
            <?php foreach ($programs as $prog): ?>
              <option value="<?= htmlspecialchars($prog) ?>" <?= ($_POST['program'] ?? '') === $prog ? 'selected' : '' ?>>
        ...
        <div class="mb-3">
          ...
          <input type="email" name="reply_to" class="form-control" placeholder="replies@mail.flypiston.com" value="<?= htmlspecialchars($_POST['reply_to'] ?? 'replies@mail.flypiston.com') ?>">
        </div>

        <div class="mb-3">
          <label class="form-label">Message</label>
          <textarea name="message" id="message" class="form-control"><?= htmlspecialchars($_POST['message'] ?? '') ?></textarea>
        </div>
    ...
    <div class="tab-pane fade" id="smsTab">
      <form method="POST">
        ...
        <div class="mb-3">
          ...
          <textarea name="sms_message" rows="4" class="form-control" required><?= htmlspecialchars($_POST['sms_message'] ?? '') ?></textarea>
