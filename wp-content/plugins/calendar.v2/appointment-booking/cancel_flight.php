...
$stmt = $conn->prepare("INSERT INTO wp_canceled_flights 
  (student_name, student_phone, flight_type, cfi_id, tail_number, home_airport, appointment_date, date_booked)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
if (!empty($flight['google_event_id']) && !empty($flight['cfi_id'])) {
  $result = deleteGoogleCalendarEvent($conn, $flight['cfi_id'], $flight['google_event_id']);
  file_put_contents(__DIR__ . '/cancel_debug.log', "📤 Calendar delete result for Flight $flightId: " . json_encode($result) . PHP_EOL, FILE_APPEND);
}
...
if (!empty($flight['cfi_id'])) {
  $stmt = $conn->prepare("SELECT first_name, last_name, email, phone FROM wp_cfis WHERE cfi_id = ?");
...
// === SEND EMAILS ===
$emailPayload = [
  'flight_type' => $flight['flight_type'] . " (CANCELLED)",
  ...
  'end' => $flight['end_time'],
  'canceled_reason' => 'Canceled via Student Portal',
  'deleted_by' => 'Student',
  'student_requested' => true
];

if (!empty($student['email'])) {
  sendEmailNotification($student['email'], $emailPayload);
}
if (!empty($cfiEmail)) {
  sendEmailNotification($cfiEmail, $emailPayload);
}

echo json_encode(['success' => true, 'flight_id' => $flightId]);
