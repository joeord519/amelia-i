...
// ✅ Fetch Aircraft List (Based on Existing Pages)
$aircraft_pages = get_pages(array(
    'post_type' => 'page',
    'meta_key' => '_wp_page_template',
    'meta_value' => 'aircraft-docs-template.php'
));
$aircraft_tail_numbers = [];
foreach ($aircraft_pages as $page) {
    $aircraft_tail_numbers[] = strtoupper(str_replace(' Aircraft Documents', '', $page->post_title));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['aircraft_doc'])) {
    ...
    // ✅ Generate File Name
    $file_name = ($category === 'Miscellaneous' && $custom_name) ? "$custom_name.$file_ext" : "$category.$file_ext";
...
$doc_categories = [
    ...
    "Last Annual" => "Last 100 Hr / Annual Entries",
...
<body>
    ...
    <script>
        document.getElementById('doc-category').addEventListener('change', function() {
            document.getElementById('custom-name-group').style.display = this.value === 'Miscellaneous' ? 'block' : 'none';
        });
    </script>
